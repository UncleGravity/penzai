...
fn finish(
    ...
    // them by patching device_encode_ns in place at its named wire offset (pinned by
    // a golden test in wire.zig) so the host can keep its transport bucket as pure
    // wire time.
    if (build_options.enable_profiling) {
