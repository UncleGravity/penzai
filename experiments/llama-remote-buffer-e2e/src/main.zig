...
fn bufClear(buf: c.ggml_backend_buffer_t, value: u8) callconv(.c) void {
    const base: [*]u8 = @ptrCast(@alignCast(remoteOf(buf).base.?));
...
fn devSupportsOp(dev: c.ggml_backend_dev_t, op: ?*const c.ggml_tensor) callconv(.c) bool {
    ...
    // Non-host preallocated weights have GGML_OP_NONE; ggml's scheduler still
    // asks whether the owning backend can handle them.
...
pub fn main() !void {
    ...
    std.debug.print("llama remote buffer e2e\nmodel: {s}\n", .{model_path});
    ...
    if (counters.non_host_buffer_allocs == 0) return E2eError.RemoteBufferNotAllocated;
    if (counters.remote_upload_bytes == 0) return E2eError.RemoteUploadMissing;
    if (counters.remote_download_bytes == 0) return E2eError.RemoteDownloadMissing;
    if (diff > 0.0001) return E2eError.LogitMismatch;

    std.debug.print("PASS: llama_decode used the Zig non-host buffer backend and matched CPU logits\n", .{});
}
