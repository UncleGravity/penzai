...
const Counters = struct {
    ...
    rejected_none: usize = 0,
    accepted_mul_mat: usize = 0,
    accepted_metadata: usize = 0,
    accepted_none: usize = 0,
    offloaded_mul_mat: usize = 0,
    offload_rejected: usize = 0,
    offload_metadata_rejected: usize = 0,
    graph_compute_calls: usize = 0,
    graph_nodes: usize = 0,
    graph_mul_mat_nodes: usize = 0,
    graph_metadata_nodes: usize = 0,
    graph_reshape_nodes: usize = 0,
    graph_permute_nodes: usize = 0,
    graph_view_nodes: usize = 0,
    graph_transpose_nodes: usize = 0,
...
fn backendGraphCompute(backend: c.ggml_backend_t, cgraph: ?*c.ggml_cgraph) callconv(.c) c.ggml_status {
    ...
    while (i < n) : (i += 1) {
        ...
        if (node.*.op == c.GGML_OP_MUL_MAT) {
            ...
            if (node.*.op == c.GGML_OP_RESHAPE) counters.graph_reshape_nodes += 1;
            if (node.*.op == c.GGML_OP_PERMUTE) counters.graph_permute_nodes += 1;
            if (node.*.op == c.GGML_OP_VIEW) counters.graph_view_nodes += 1;
            if (node.*.op == c.GGML_OP_TRANSPOSE) counters.graph_transpose_nodes += 1;
        } else {
...
fn devSupportsOp(dev: c.ggml_backend_dev_t, op: ?*const c.ggml_tensor) callconv(.c) bool {
    ...
    if (t.op == c.GGML_OP_NONE) counters.rejected_none += 1;
    return false;
}
...
fn devOffloadOp(dev: c.ggml_backend_dev_t, op: ?*const c.ggml_tensor) callconv(.c) bool {
    ...
    counters.offload_rejected += 1;
    if (isMetadataOp(t.op)) counters.offload_metadata_rejected += 1;
    return false;
}
...
pub fn main() !void {
    ...
    std.debug.print("llama partial offload\nmodel: {s}\n", .{model_path});
    ...
    std.debug.print(
        ...
        .{
            ...
            counters.offload_rejected,
            counters.offload_metadata_rejected,
            counters.graph_compute_calls,
            counters.graph_nodes,
            counters.graph_mul_mat_nodes,
            counters.graph_metadata_nodes,
            counters.graph_reshape_nodes,
            counters.graph_permute_nodes,
            counters.graph_view_nodes,
            counters.graph_transpose_nodes,
    ...
    if (counters.graph_mul_mat_nodes == 0) return E2eError.BackendComputeNotCalled;
    if (counters.graph_unexpected_nodes != 0) return E2eError.UnexpectedOpOffloaded;
    if (diff > 0.0001) return E2eError.LogitMismatch;

    std.debug.print("PASS: llama_decode assigned MUL_MAT with metadata-only graph glue to the Zig ggml backend and matched CPU logits\n", .{});
}
