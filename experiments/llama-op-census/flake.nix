      ...
      perSystem = { self', system, ... }:
        let
          ...
          e2e = pkgs.stdenv.mkDerivation {
            ...
            installPhase = ''
              runHook preInstall
              mkdir -p "$out/bin"
              cp zig-out/bin/llama-op-census "$out/bin/"
              runHook postInstall
            '';
        ...
        in {
          ...
          apps = {
            default = {
              type = "app";
              program = "${self'.packages.default}/bin/llama-op-census";
            };
            e2e = {
              type = "app";
              program = "${self'.packages.e2e}/bin/llama-op-census";
            };
          };
