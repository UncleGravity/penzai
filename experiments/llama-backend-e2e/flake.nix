{
  description = "penzai llama.cpp custom backend e2e experiment";
      ...
      perSystem = { self', system, ... }:
        let
          ...
          e2e = pkgs.stdenv.mkDerivation {
            pname = "penzai-llama-backend-e2e";
            version = "0.1.0";
            src = ./.;
            ...
            installPhase = ''
              runHook preInstall
              mkdir -p "$out/bin"
              cp zig-out/bin/llama-backend-e2e "$out/bin/"
              runHook postInstall
            '';
        ...
        in {
          ...
          apps = {
            default = {
              type = "app";
              program = "${self'.packages.default}/bin/llama-backend-e2e";
            };
            e2e = {
              type = "app";
              program = "${self'.packages.e2e}/bin/llama-backend-e2e";
            };
          };
