//! AXI DMA direct-register driver for the KR260 multiport bandwidth fixture.
...
pub const Dma = struct {
    ...
    pub fn openDevice(base: i64) !Dma {
    ...
    /// Reset only the MM2S channel — for a DMA built without S2MM (the acts lane).
    pub fn resetMm2s(self: *Dma) !void {
        self.regs[MM2S_DMACR] = RESET;
        try self.waitResetClear(MM2S_DMACR, "MM2S");
    }
    ...
    pub fn snapshot(self: *Dma) Status {
        return .{
            .mm2s = self.regs[MM2S_DMASR],
            .s2mm = self.regs[S2MM_DMASR],
        };
    }
