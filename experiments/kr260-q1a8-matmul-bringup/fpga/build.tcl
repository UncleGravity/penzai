# build.tcl - KR260 Q1A8 matmul bring-up bitstream.
#
# Design (M3): one q1a8_kernel_top fed by two AXI DMAs.
#   PS pl_clk0 @ ~100 MHz -> clk_wiz -> fabric clock (200 or 300 MHz)
#   PS M_AXI_HPM0_FPD -> AXI-Lite -> dma_w + dma_a + kernel
#   dma_w MM2S: DDR -> kernel.S_AXIS        (weights)
...
#
# Datapath is 64-bit to match the kernel AXIS width. Bandwidth is NOT the point
# here (correctness on silicon is); the wide-array / multi-HP work is M6.

set variant [expr {$argc >= 1 ? [lindex $argv 0] : "w64-f100"}]
...
# higher clocks need the M6 datapath pipelining (or a multicycle constraint).
if {![regexp {^w64-f([0-9]+)$} $variant -> fclk_mhz]} {
    error "unknown variant '$variant'; expected w64-f<MHz>, e.g. w64-f100"
}
...
proc print_width_props {label obj} {
    puts "DIAG: width props for $label"
    foreach prop [list_property $obj] {
        if {[regexp -nocase {width|tdata} $prop]} {
            if {![catch {set value [get_property $prop $obj]}]} { puts "  $prop = $value" }
        }
    }
}
...
# So run the DMAs at the proven 128-bit width (== multiport) and bridge to the
# 64-bit kernel with AXIS data-width converters, which pack/unpack byte-exactly.
...
make_dwc dwc_w 16 8 ;# weights  128 -> 64
make_dwc dwc_a 16 8 ;# acts     128 -> 64
make_dwc dwc_r 8 16 ;# results   64 -> 128

create_bd_cell -type module -reference q1a8_kernel_top kernel
...
print_width_props "kernel/S_AXIS"      [get_bd_intf_pins kernel/S_AXIS]
print_width_props "kernel/S_AXIS_ACTS" [get_bd_intf_pins kernel/S_AXIS_ACTS]
print_width_props "kernel/M_AXIS"      [get_bd_intf_pins kernel/M_AXIS]
print_width_props "dma_w"              [get_bd_cells dma_w]
print_width_props "dma_a"              [get_bd_cells dma_a]
print_width_props "ps/S_AXI_HP0_FPD"   [get_bd_intf_pins ps/S_AXI_HP0_FPD]
print_width_props "ps/S_AXI_HP1_FPD"   [get_bd_intf_pins ps/S_AXI_HP1_FPD]
