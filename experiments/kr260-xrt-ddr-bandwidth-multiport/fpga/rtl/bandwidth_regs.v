module bandwidth_regs (
    ...
    wire aw_take = !s_axi_bvalid && !aw_pending && s_axi_awvalid;
    wire w_take = !s_axi_bvalid && !w_pending && s_axi_wvalid;
    wire write_fire = !s_axi_bvalid && (aw_pending || aw_take) && (w_pending || w_take);
    wire [5:0] write_addr = aw_pending ? awaddr_hold : s_axi_awaddr;
    ...
    always @(posedge aclk) begin
        if (!aresetn) begin
            ...
            s_axi_awready <= !s_axi_bvalid && !aw_pending;
            s_axi_wready <= !s_axi_bvalid && !w_pending;

            if (write_fire) begin
                case (write_addr[5:2])
                    REG_CONTROL: begin
                        if (write_data[0]) gen_start <= 1'b1;
                        if (write_data[1]) check_start <= 1'b1;
                    end
                    REG_LENGTH_LO: length_bytes[31:0] <= write_data;
                    REG_LENGTH_HI: length_bytes[63:32] <= write_data;
                    REG_BASE_LO:   base_index_lo <= write_data;
                    REG_SEED:      seed <= write_data[7:0];
                    default: begin end
                endcase
                aw_pending <= 1'b0;
                w_pending <= 1'b0;
    ...
    wire unused_axi = |s_axi_awaddr[1:0] | |write_addr[1:0] | |s_axi_araddr[1:0] |
