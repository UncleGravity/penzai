...
const binary_name = "kr260-xrt-ddr-bandwidth-multiport";
const dts_template_name = "penzai-ddr-bandwidth-multiport.dts";
const bit_prefix = "penzai-ddr-bandwidth-multiport";
...
pub fn build(b: *std.Build) void {
    ...
    const rtl_test = b.step("rtl-test", "Run lightweight RTL unit tests with Verilator");
    rtl_test.dependOn(&addRtlTestCommand(b).step);
...
fn addDeployCommand(b: *std.Build, variant: []const u8) *std.Build.Step.Run {
    const run = b.addSystemCommand(&.{
        ...
        config_prelude ++
            ...
            \\APP="${APP:-penzai-ddr-bandwidth-multiport}"
            \\VARIANT="${1:?missing variant}"
            \\BIT_PREFIX="${2:?missing bit prefix}"
            \\DTS_NAME="${3:?missing dts template name}"
            \\
            \\case "$BOARD_TMP" in
            \\  /tmp/*) ;;
            ...
            \\for SRC in fpga/build.tcl fpga/build.bat fpga/rtl/*.v; do
            \\  if [[ "$SRC" -nt "$BIT" ]]; then
            \\    echo "ERROR: $BIT is older than $SRC. Run 'zig build bitstream -Dvariant=$VARIANT' before deploy." >&2
            \\    exit 1
            \\  fi
            \\done
            \\
            ...
            \\ssh "$BOARD" "APP='$APP' BOARD_TMP='$BOARD_TMP' BIT_NAME='$BIT_NAME' DTS_NAME='$DTS_NAME' bash -s" <<'REMOTE'
            \\set -euo pipefail
            \\FW="/lib/firmware/xilinx/$APP"
            \\cd "$BOARD_TMP"
            \\
            \\GENERATED_DTS="$APP.generated.dts"
            \\sed "s/@FIRMWARE_NAME@/$BIT_NAME/g" "$DTS_NAME" > "$GENERATED_DTS"
    ...
    run.addArg(variant);
    run.addArg(bit_prefix);
    run.addArg(dts_template_name);
    return run;
}

fn addRtlTestCommand(b: *std.Build) *std.Build.Step.Run {
    return b.addSystemCommand(&.{
        "bash",
        "-lc",
        \\set -euo pipefail
        \\OUT=".zig-cache/rtl-test-axis-pattern-check"
        \\verilator --binary --timing -Wall --Wno-TIMESCALEMOD --Mdir "$OUT" --top-module axis_pattern_check_tb \
        \\  fpga/rtl/axis_pattern_check.v \
        \\  fpga/sim/axis_pattern_check_tb.sv
        \\"$OUT/Vaxis_pattern_check_tb"
    });
}
...
fn addRunCommand(
    ...
    const run = b.addSystemCommand(&.{
        ...
        config_prelude ++
            ...
            \\scp "$BIN" "$BOARD:$BOARD_TMP/kr260-xrt-ddr-bandwidth-multiport"
            \\
            \\echo "== run kr260-xrt-ddr-bandwidth-multiport $ARGS =="
            \\ssh "$BOARD" "BOARD_TMP='$BOARD_TMP' ARGS='$ARGS' bash -s" <<'REMOTE'
            \\set -euo pipefail
            \\chmod +x "$BOARD_TMP/kr260-xrt-ddr-bandwidth-multiport"
            \\sudo "$BOARD_TMP/kr260-xrt-ddr-bandwidth-multiport" $ARGS
            \\REMOTE
        ,
        "zig-build",
    });
