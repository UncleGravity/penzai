...
pub fn weightBytes(num_rowblocks: usize, q1_blocks: usize) usize {
    return num_rowblocks * q1_blocks * layout.WEIGHT_BYTES_PER_BLOCK;
}
pub fn actBytes(q1_blocks: usize) usize {
    return q1_blocks * layout.ACT_BYTES_PER_BLOCK;
}
pub fn resultBytes(num_rowblocks: usize) usize {
    return num_rowblocks * layout.RESULT_BYTES_PER_ROWBLOCK;
}
...
pub fn packWeights(
    ...
    const num_rowblocks = rows / layout.ROWS;
    ...
    while (rb < num_rowblocks) : (rb += 1) {
        ...
        while (blk < q1_blocks) : (blk += 1) {
            ...
            var sbeat: usize = 0;
            while (sbeat < layout.SCALE_BEATS) : (sbeat += 1) {
                var word: u64 = 0;
                var local: usize = 0;
                while (local < 4) : (local += 1) {
                    const lane = sbeat * 4 + local;
                    if (lane >= layout.ROWS) continue;
                    const s = weight_scales[(rb * layout.ROWS + lane) * q1_blocks + blk];
            ...
            while (sub < layout.Q8_SUBBLOCKS) : (sub += 1) {
                var wbeat: usize = 0;
                while (wbeat < layout.WBITS_BEATS) : (wbeat += 1) {
                    var word: u64 = 0;
                    var local: usize = 0;
                    while (local < 2) : (local += 1) {
                        const lane = wbeat * 2 + local;
                        if (lane >= layout.ROWS) continue;
                        const bits = weight_bits[(rb * layout.ROWS + lane) * q1_blocks + blk];
...
pub const WIDE_BEAT_BYTES: usize = layout.ROWS * 4; // ROWS*32 bits
        ...
        while (blk < q1_blocks) : (blk += 1) {
            ...
            while (sbeat < layout.SCALE_BEATS) : (sbeat += 1) {
                ...
                while (local < 4) : (local += 1) {
                    ...
                    out_scales[(rb * layout.ROWS + lane) * q1_blocks + blk] = @bitCast(sb);
                }
            }
            // accumulate bits per (rowblock-lane); zero them first this block
            var lane: usize = 0;
            while (lane < layout.ROWS) : (lane += 1)
                out_bits[(rb * layout.ROWS + lane) * q1_blocks + blk] = 0;
            var sub: usize = 0;
            while (sub < layout.Q8_SUBBLOCKS) : (sub += 1) {
                var wbeat: usize = 0;
                while (wbeat < layout.WBITS_BEATS) : (wbeat += 1) {
                    ...
                    while (local < 2) : (local += 1) {
                        ...
                        if (l >= layout.ROWS) continue;
                        const wb: u128 = @as(u32, @truncate(word >> @intCast(local * 32)));
                        out_bits[(rb * layout.ROWS + l) * q1_blocks + blk] |= wb << @intCast(sub * 32);
                    }
                }
            }
        }
...
pub fn quantizeActs(column: []const f32, out_quants: []i8, out_scales: []f16) void {
    const nblocks = column.len / layout.Q8_BLOCK;
    std.debug.assert(out_quants.len == column.len);
    std.debug.assert(out_scales.len == nblocks);
    var b: usize = 0;
    while (b < nblocks) : (b += 1) {
        const base = b * layout.Q8_BLOCK;
        var amax: f32 = 0;
        for (column[base..][0..layout.Q8_BLOCK]) |v| amax = @max(amax, @abs(v));
        if (amax == 0) {
            out_scales[b] = 0;
            @memset(out_quants[base..][0..layout.Q8_BLOCK], 0);
            continue;
        }
        const scale: f16 = @floatCast(amax / 127.0);
        out_scales[b] = scale;
        const inv: f32 = 1.0 / @as(f32, @floatCast(scale));
        for (column[base..][0..layout.Q8_BLOCK], 0..) |v, i| {
...
pub fn packActs(q1_blocks: usize, quants: []const i8, scales: []const f16, out: []u8) void {
    ...
    while (blk < q1_blocks) : (blk += 1) {
        ...
        while (sub < layout.Q8_SUBBLOCKS) : (sub += 1) {
            ...
            var beat: usize = 0;
            while (beat < layout.Q8_BLOCK / layout.BEAT_BYTES) : (beat += 1) {
                var word: u64 = 0;
                var byte: usize = 0;
                while (byte < layout.BEAT_BYTES) : (byte += 1) {
...
pub fn unpackResults(num_rowblocks: usize, bytes: []const u8, out: []f32) void {
    ...
    std.debug.assert(out.len == num_rowblocks * layout.ROWS);
    var off: usize = 0;
    var rb: usize = 0;
    while (rb < num_rowblocks) : (rb += 1) {
        var beat: usize = 0;
        while (beat < layout.ROWS / 2) : (beat += 1) {
            const word = get(bytes, &off);
            const lane = rb * layout.ROWS + beat * 2;
...
test "weight pack/unpack roundtrip" {
    const rows = layout.ROWS * 2; // 2 rowblocks
    const blocks = 5;
...
test "quantizeActs scale and reconstruction" {
    var col: [layout.Q1_BLOCK]f32 = undefined;
    for (&col, 0..) |*v, i| v.* = @floatFromInt(@as(i32, @intCast(i)) - 64);
    var q: [layout.Q1_BLOCK]i8 = undefined;
    var s: [layout.Q8_SUBBLOCKS]f16 = undefined;
    quantizeActs(&col, &q, &s);
    // dequant should track the input within one quantization step
    for (col, 0..) |v, i| {
        const dq = @as(f32, @floatFromInt(q[i])) * @as(f32, @floatCast(s[i / layout.Q8_BLOCK]));
        try testing.expectApproxEqAbs(v, dq, @as(f32, @floatCast(s[i / layout.Q8_BLOCK])) + 1e-3);
    }
}

test "result pack/unpack roundtrip" {
    const num_rowblocks = 2;
    var vals: [num_rowblocks * layout.ROWS]f32 = undefined;
    for (&vals, 0..) |*v, i| v.* = @as(f32, @floatFromInt(i)) * 1.25 - 3.0;
    var buf: [resultBytes(num_rowblocks)]u8 = undefined;
    var off: usize = 0;
    for (0..num_rowblocks) |rb| {
        for (0..layout.ROWS / 2) |beat| {
            const lo: u64 = @as(u32, @bitCast(vals[rb * layout.ROWS + beat * 2]));
            const hi: u64 = @as(u32, @bitCast(vals[rb * layout.ROWS + beat * 2 + 1]));
            put(&buf, &off, lo | (hi << 32));
        }
    }
    var out: [num_rowblocks * layout.ROWS]f32 = undefined;
