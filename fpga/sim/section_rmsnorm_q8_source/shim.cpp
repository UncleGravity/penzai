...
#include "Vsection_rmsnorm_q8_source.h"
#include "Vsection_rmsnorm_q8_source___024root.h"
#include "verilated.h"

struct Dut {
    Vsection_rmsnorm_q8_source *top;
};
...
Dut *dut_new(void) {
    Dut *d = new Dut();
    d->top = new Vsection_rmsnorm_q8_source();
...
int dut_debug_scalar_fire(Dut *d) {
    return d->top->rootp->section_rmsnorm_q8_source__DOT__debug_weighted_scalar_fire;
}
int dut_debug_source_done(Dut *d) {
    return d->top->rootp->section_rmsnorm_q8_source__DOT__debug_weighted_done;
}
uint8_t dut_debug_wrapper_state(Dut *d) {
    return d->top->rootp->section_rmsnorm_q8_source__DOT__debug_state;
}
uint8_t dut_debug_source_state(Dut *d) {
    return d->top->rootp->section_rmsnorm_q8_source__DOT__debug_weighted_state;
}
...
int dut_debug_q8_record_done(Dut *d) {
    return d->top->rootp->section_rmsnorm_q8_source__DOT__debug_q8_record_done;
}
