#include "Vsection_residual_add_rne.h"
#include "Vsection_residual_add_rne___024root.h"
#include "verilated.h"
#include "shim.h"

struct Dut {
    Vsection_residual_add_rne *top;
};
...
Dut *dut_new(void) {
    Dut *d = new Dut();
    d->top = new Vsection_residual_add_rne();
    return d;
}
...
uint16_t dut_debug_exponent(Dut *d) {
    return d->top->rootp->section_residual_add_rne__DOT__exponent_q;
}
uint32_t dut_debug_small_ext(Dut *d) {
    return d->top->rootp->section_residual_add_rne__DOT__small_ext_q;
}
uint32_t dut_debug_magnitude(Dut *d) {
    return d->top->rootp->section_residual_add_rne__DOT__magnitude_q;
}
