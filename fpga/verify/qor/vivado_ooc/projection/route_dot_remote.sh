...
period_ns="${1:-3.333}"
out_dir="${2:-$repo/.zig-cache/fpga-verify/vivado-ooc/dot4-remote}"
...
stage="$(mktemp -d "${TMPDIR:-/tmp}/dot4-route.XXXXXX")"
trap 'rm -rf "$stage"' EXIT
mkdir -p "$out_dir"
cp "$qor_dir/route_dot.tcl" "$qor_dir/route_dot.bat" "$stage/"
cp "$repo/fpga/rtl/projection/dot4.v" "$stage/"
cp "$repo/fpga/verify/qor/vivado_ooc/projection/dot4_ooc.v" "$stage/"

printf 'sha256\tbytes\tpath\n' > "$out_dir/resolved_sources.tsv"
for file in dot4.v dot4_ooc.v route_dot.tcl route_dot.bat; do
  printf '%s\t%s\t%s\n' \
    "$(shasum -a 256 "$stage/$file" | awk '{print $1}')" \
    "$(wc -c < "$stage/$file" | tr -d ' ')" "$file" \
    >> "$out_dir/resolved_sources.tsv"
done

run_id="dot4-$(date -u +%Y%m%dT%H%M%SZ)-$$"
remote_dir="$VM_OOC/$run_id"
remote_dir_win="$VM_OOC\\$run_id"

echo "==> remote dot4 OOC route: $VM:$remote_dir period=${period_ns}ns"
...
set +e
ssh "${ssh_args[@]}" "$VM" \
  "cd $remote_dir && route_dot.bat $period_ns result ." \
