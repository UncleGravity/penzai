...
//
// Pure feed-forward: widen → multiply → pipelined adder tree, composed from shared
...
// m/l/acc). That keeps recurrence-hiding in one place rather than duplicating
// accumulator scheduling inside the leaf.
//
...
// MUL_PIPE=1 to split the fp32 two-DSP multiply. The extra stage is pure
...
module fp_dot (
    ...
    generate
        for (i = 0; i < LANES; i = i + 1) begin : gen_mul
            // MUL_PIPE=1: the fp32 24×24 mantissa multiply is a two-DSP cascade; the extra
            // stage splits it to one DSP per cycle. The kernel is
    ...
    // Pipelined fp32 adder tree 8 → 1 (same (0,1)(2,3)… pairing order).
