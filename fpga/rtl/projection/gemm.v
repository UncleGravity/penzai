// DSP-resident fixed-point projection GEMM.
//
// The front end reduces each packed-weight sub-block against the shared Q8
// activations, decomposes the f16 scales, and feeds exact wide accumulators.
// Normalization occurs once per output. Scale decomposition remains local to
// this datapath because the attention path only needs f16-to-f32 widening.
...
// gemm_emit - per-output fixed-to-float conversion. The wide accumulator has
// value acc * 2^emin and is normalized once into truncated fp32.
//
...
// fmul/fadd/cvt), and bias the exponent by (msb + emin).
//
// Pipelined over GEMM_EMIT_LATENCY=4 stages
// (negate | leading-zero-detect | shift+exponent | compose):
// each stage isolates one major combinational block. valid_in -> valid_out tracks
// the pipeline; consumers emit once per output, outside the inner recurrence.
module gemm_emit #(
    ...
    // ---- s3: shift to the window + exponent ----
    ...
    // ---- s4: range checks + FP32 compose ----
    wire underflow3 = (e_b3 <= 16'sd0);
    wire overflow3  = (e_b3 >= 16'sd255);
    wire [31:0] f32_c = zero3      ? 32'd0
                      : underflow3 ? {sign3, 31'd0}
                      : overflow3  ? {sign3, 8'hFE, 23'h7FFFFF}   // saturate to max finite
                                   : {sign3, e_b3[7:0], mant3};
    always @(posedge clk) begin
        if (!rst_n) begin valid_out<=1'b0; f32<=32'd0; end
        else begin valid_out <= v3; f32 <= f32_c; end
    end
endmodule
...
// axis. Accumulation is a SINGLE-CYCLE saturating add: re-issuing the same
...
module gemm_rowblock #(
    ...
    // Fixed-point contract shared with fma's defaults.
    ...
    // The signed activation front end is pipelined over FE_LAT stages so the
    // 32-element add tree closes timing. Scale significands and control metadata
    // are delayed by the same depth to meet the sub-sum at fma_mulshift.
    //
    // FE_LAT=4 = ONE input-register stage (acts_r/wb_r) + the 3-deep quad pipeline. The
    // input stage gives the activation-memory read its own cycle. Scale exponents
    // are registered before their add, then use one fewer delay
    ...
    // Register the quad-sum inputs so the activation BRAM read and the sum tree do not
    // share a cycle.
    reg [255:0]       acts_r;
    ...
    generate
        for (r = 0; r < ROWS; r = r + 1) begin : gen_lane
            ...
            // path. The one real add (accS+accC) happens once per
            ...
            // readout latency, which the projection engine realigns (pc_beat/pc_vld). The accumulate
