// Complete scratch-backed weighted RMSNorm scalar pipeline.
//
// Reduction either loads an external residual into R scratch or scans an
// already-resident R allocation. The weighted source consumes the resulting
// inverse-RMS records, rereads R, and publishes one FP32 scalar stream. This
// wrapper owns the untagged scratch response across both phases and closes the
// two child lifecycles as one restart-safe command.
...
module section_rmsnorm_scalar_pipeline #(
    ...
    output reg           done,
    output reg           error,
    // bits 12:0 reduction raw status, bits 21:13 weighted-source raw
    // status, bit 22 wrapper lifecycle/protocol status.
...
`ifdef FORMAL
    ...
    , output wire [8:0]  formal_source_status
    ...
    , output wire        formal_final_output_seen
    , output wire        formal_fault_latched
    , output wire [22:0] formal_latched_status
...
`endif
    ...
    reg [1:0] output_token_q;
    reg [8:0] output_block_q;
    reg [5:0] output_scalar_q;
    reg reduce_done_seen_q;
    reg source_done_seen_q;
    reg final_output_seen_q;
    reg fault_latched_q;
    reg report_error_q;
    reg cross_abort_q;
    reg cleanup_abort_issued_q;
    reg [22:0] latched_status_q;
    ...
    wire [31:0] source_scalar_data;
    wire source_scalar_valid;
    wire source_scalar_ready;
    wire source_scalar_last;
    wire [1:0] source_scalar_status;
    ...
    section_rmsnorm_reduce u_reduce (
        .clk(clk), .rst_n(rst_n),
        .cfg_valid(cfg_fire), .cfg_ready(reduce_cfg_ready),
        .cfg_rows(cfg_rows), .cfg_tokens(cfg_tokens), .cfg_eps(cfg_eps),
        .cfg_resident(cfg_resident), .abort_run(child_abort),
        .busy(reduce_busy), .done(reduce_done), .error(reduce_error),
        .status(reduce_status), .s_axis_tdata(s_axis_tdata),
        .s_axis_tkeep(s_axis_tkeep),
        .s_axis_tvalid(s_axis_tvalid && traffic_enable),
        .s_axis_tready(reduce_s_axis_tready), .s_axis_tlast(s_axis_tlast),
        .r_wr_valid(reduce_r_wr_valid), .r_wr_ready(reduce_r_wr_ready),
        .r_wr_error(r_wr_error), .r_wr_bank(reduce_r_wr_bank),
        .r_wr_address(reduce_r_wr_address), .r_wr_data(reduce_r_wr_data),
    ...
    section_rmsnorm_weighted_source #(
        ...
        .gamma_tready(source_gamma_tready), .gamma_tlast(gamma_tlast),
        ...
        .cfg_ready(source_cfg_ready), .cfg_rows(cfg_rows),
        .cfg_tokens(cfg_tokens), .busy(source_busy), .done(source_done),
        .error(source_error), .status(source_status),
        .inv_valid(reduce_result_valid && traffic_enable),
        .inv_ready(source_inv_ready), .inv_token(reduce_result_token),
        .inv_rms(reduce_result_inv_rms), .inv_final(reduce_result_final),
        ...
        .rd_rsp_data(rd_rsp_data), .rd_rsp_error(rd_rsp_error),
        .scalar_valid(source_scalar_valid),
        .scalar_ready(source_scalar_ready), .scalar_data(source_scalar_data),
        .scalar_last(source_scalar_last),
        .scalar_status(source_scalar_status)
    );
    ...
    wire can_consider_read = run_window && (scratch_owner_q == RD_NONE);
    wire scratch_request_conflict = can_consider_read &&
                                    reduce_rd_req_valid &&
                                    source_rd_req_valid;
    wire orphan_response = run_window && (scratch_owner_q == RD_NONE) &&
                           rd_rsp_valid;
    ...
    assign source_scalar_ready = scalar_ready && traffic_enable;
    assign scalar_data = source_scalar_data;
    assign scalar_valid = source_scalar_valid && traffic_enable;
    assign scalar_last = source_scalar_last && traffic_enable;
    assign scalar_status = source_scalar_status;
    wire output_fire = source_scalar_valid && source_scalar_ready;
    wire expected_scalar_last = output_scalar_q == 6'd31;
    wire output_frame_bad = output_fire &&
                            (source_scalar_last != expected_scalar_last);
    wire final_output_fire = output_fire && source_scalar_last &&
                             ({1'b0, output_token_q} + 3'd1 ==
                              run_tokens_q) &&
                             (output_block_q == last_block_q);

    assign busy = state_q != ST_IDLE;
    wire [22:0] child_status_now = {1'b0, source_status, reduce_status};
    wire [22:0] diagnosed_child_status =
        (child_status_now != 23'd0) ? child_status_now : STATUS_INTERNAL;
    wire [22:0] fault_status_now =
        (child_fault_now ? diagnosed_child_status : 23'd0) |
        (internal_fault_now || output_frame_bad ? STATUS_INTERNAL : 23'd0);
                ...
                case (state_q)
                    ...
                    ST_RUN: begin
                        if (output_fire) begin
                            if (source_scalar_last) begin
                                output_scalar_q <= 6'd0;
                                if (final_output_fire) begin
                                    final_output_seen_q <= 1'b1;
                                end else if (output_block_q == last_block_q) begin
                                    output_block_q <= 9'd0;
                                    output_token_q <= output_token_q + 1'b1;
                                end else begin
                                    output_block_q <= output_block_q + 1'b1;
                                end
                            end else begin
                                output_scalar_q <= output_scalar_q + 1'b1;
                            end
                        end

                        if (child_fault_now || internal_fault_now ||
                            output_frame_bad) begin
                            state_q <= ST_CLEANUP;
                            fault_latched_q <= 1'b1;
                            report_error_q <= 1'b1;
                            cross_abort_q <= 1'b1;
...
`ifdef VERILATOR
    ...
    wire debug_child_fault /* verilator public_flat_rd */ =
        child_fault_now || internal_fault_now || output_frame_bad;
...
`ifdef FORMAL
    ...
    assign formal_internal_fault = internal_fault_now || output_frame_bad;
    ...
    assign formal_final_output_seen = final_output_seen_q;
