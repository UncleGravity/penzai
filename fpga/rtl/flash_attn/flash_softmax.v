...
//
// Mirrors device/ps/flash_attn.zig + flash_ref exactly. The kernel holds the
// per-head (m,l) pool and feeds m_out/l_out back as the next kv's m_in/l_in; the
// loop-carried recurrence is closed there and hidden by the n_heads interleave, so
// this unit is purely feed-forward (no internal state). The kernel always applies
