    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                // One tile of Q is resident for the entire KV-outer walk. Stream order
                // is query, head, beat.
                ...
                S_PAIR: begin
                    ...
                    // P2c softmax is a stateless II=1 pipe. Slot tags stay ordered even
                ...
                S_KVNEXT: if (last_kv) begin
                    ...
                    kv_i <= kv_i + 16'd1; ld_tok <= 0; mask_finite <= 0; state <= S_MASK;
                end

                // l==0 is the all-masked query contract: emit exact zero and never
                // wait for or use recip(0). Other rows retain the P2b finalization.
                S_FRECF: begin
                    bi <= 0;
