...
# should report VERSION 8 with counters.
#
#   ./deploy.sh                         # uses VARIANT from config.env
#   ./deploy.sh w512-p4-f125-wc250
...
BIT="out/$BIT_NAME"
DTS_TEMPLATE="overlay/penzai-q1a8-mc.dts"
...
sed "s/@FIRMWARE_NAME@/$BIT_NAME/g" penzai-q1a8-mc.dts > "$GENERATED_DTS"
...
REMOTE
echo "Done. Restart penzaid (as root) and check its PL init line reports version 8."
