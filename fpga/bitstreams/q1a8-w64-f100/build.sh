...
#
# Syncs the v5 narrow RTL + generated regmap header + the TCL/BAT to the VM,
...
#
#   ./build.sh            # uses VARIANT from config.env (default w64-f100)
#   ./build.sh w64-f100   # explicit variant
#
...
# v5 narrow kernel RTL set (excludes the wide kernel, which this design does not
# instantiate). q1a8_regs.vh must exist — generate with `zig build regmap`.
RTL_FILES=(
  "$RTL_DIR/q1a8_kernel_top.v"
  "$RTL_DIR/q1a8_kernel.v"
  "$RTL_DIR/q1a8_rowblock.v"
  "$RTL_DIR/q1a8_reducer.v"
  "$RTL_DIR/fp32_add.v"
  "$RTL_DIR/fp32_mul.v"
