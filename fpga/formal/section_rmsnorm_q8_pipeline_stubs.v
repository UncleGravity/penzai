...
// Short-latency control abstraction of the independently proven exact FP32
// multiplier. Results remain retained under backpressure and abort discards all
// tentative state; numeric identity is deliberately noncommutative.
...
// scalars and retains one complete native record until the fifth serialized
// beat releases it through out_ready.
module q8_quantizer (
    input  wire         clk,
    input  wire         rst_n,
    input  wire         in_valid,
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (input_fire) begin
                ...
                fault_q <= fault_q || (in_data == FAULT_WORD);
