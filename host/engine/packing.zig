//! Host-side conversion from GGUF low-bit blocks to the resident engine image.
...
// Resident weight layout for the four-port projection engine. Each rowblock has 16
...
// Activation-BRAM capacity of the projection engine. The engine stores activations indexed by
...
// would alias. 512 puts the cap at K=16384, the exact limit of the accumulator
// window. This constant is part of the resident-image/RTL contract.
...
fn fusedGateUpRows(rows: usize) LayoutError!usize {
    if (rows == 0 or rows > std.math.maxInt(usize) / 2) return error.InvalidRows;
    return rows * 2;
}

const FusedQkvShape = struct {
    q_rows: usize,
    kv_rows: usize,
    rows: usize,
...
const FusedQkvSource = struct {
    tensor: enum { q, k, v },
    row: usize,
};

fn fusedQkvShape(q_heads: usize, kv_heads: usize, head_dim: usize) LayoutError!FusedQkvShape {
    if (q_heads == 0 or kv_heads == 0 or head_dim == 0 or q_heads % kv_heads != 0)
        return error.InvalidRows;
    const q_rows = std.math.mul(usize, q_heads, head_dim) catch return error.InvalidRows;
    const kv_rows = std.math.mul(usize, kv_heads, head_dim) catch return error.InvalidRows;
    const qk_rows = std.math.add(usize, q_rows, kv_rows) catch return error.InvalidRows;
    const rows = std.math.add(usize, qk_rows, kv_rows) catch return error.InvalidRows;
    return .{
        .q_rows = q_rows,
        .kv_rows = kv_rows,
        .rows = rows,
...
fn fusedQkvSourceRow(
    shape: FusedQkvShape,
    head_dim: usize,
    fused_row: usize,
) FusedQkvSource {
    const q_group_rows = shape.q_heads_per_kv * head_dim;
    const group_rows = q_group_rows + 2 * head_dim;
    const kv_head = fused_row / group_rows;
    const local = fused_row % group_rows;
    if (local < q_group_rows)
        return .{ .tensor = .q, .row = kv_head * q_group_rows + local };
    if (local < q_group_rows + head_dim)
        return .{ .tensor = .k, .row = kv_head * head_dim + local - q_group_rows };
    return .{
        .tensor = .v,
        .row = kv_head * head_dim + local - q_group_rows - head_dim,
    };
}

/// Resident bytes for GATE and UP packed as logical rows
/// `[G0, U0, G1, U1, ...]`. The four-port zip preserves this order, so every
/// adjacent GEMM output lane is a matching `{gate, up}` pair.
pub fn packedFusedGateUpWeightBytes(rows: usize, k: usize) LayoutError!usize {
    return packedWeightBytes(try fusedGateUpRows(rows), k);
}

pub fn packedFusedGateUpTernaryWeightBytes(rows: usize, k: usize) LayoutError!usize {
    return packedTernaryWeightBytes(try fusedGateUpRows(rows), k);
}

/// Resident bytes for one QKV projection launch. Logical rows are contiguous
/// `[Q..., K..., V...]`; the packer below converts those three source tensors
/// directly into the four-port rowblock layout.
pub fn packedFusedQkvWeightBytes(
    q_heads: usize,
    kv_heads: usize,
    head_dim: usize,
    input_k: usize,
) LayoutError!usize {
    return packedWeightBytes((try fusedQkvShape(q_heads, kv_heads, head_dim)).rows, input_k);
}

pub fn packedFusedQkvTernaryWeightBytes(
    q_heads: usize,
    kv_heads: usize,
    head_dim: usize,
    input_k: usize,
) LayoutError!usize {
    return packedTernaryWeightBytes((try fusedQkvShape(q_heads, kv_heads, head_dim)).rows, input_k);
}
...
/// Pack separate GATE and UP Q2_0 matrices directly into fused resident order.
pub fn packFusedGateUpWeightsFromGgmlQ2_0(
    rows: usize,
    k: usize,
    ...
    const fused_rows = try fusedGateUpRows(rows);
    const q1_blocks = try blocksPerRow(k);
    const source_blocks_per_row = k / q2_source_block;
    const source_row_bytes = source_blocks_per_row * q2_source_block_bytes;
    const source_bytes = rows * source_row_bytes;
    if (gate_weights.len != source_bytes or up_weights.len != source_bytes) return error.InvalidLength;
    if (out.len != try packedTernaryWeightBytes(fused_rows, k)) return error.InvalidLength;

    @memset(out, 0);
    for (0..weight_ports) |port| {
        for (0..rowblocksFor(fused_rows)) |rb| {
            const row_start = rb * rows_per_block + port * rows_per_port;
            const row_count = if (row_start < fused_rows) @min(rows_per_port, fused_rows - row_start) else 0;
            for (0..q1_blocks) |q1| {
                const block_base = ternaryWeightBlockOffset(fused_rows, k, port, rb, q1);
                for (0..row_count) |lane| {
                    const fused_row = row_start + lane;
                    const source = if (fused_row & 1 == 0) gate_weights else up_weights;
                    const source_row = fused_row / 2;
                    const src0 = source_row * source_row_bytes + (q1 * 2) * q2_source_block_bytes;
                    const src1 = src0 + q2_source_block_bytes;
                    for (0..2) |half| {
                        const source_base = if (half == 0) src0 else src1;
                        const scale = std.mem.readInt(u16, source[source_base..][0..2], .little);
                        std.mem.writeInt(u16, out[block_base + lane * 4 + half * 2 ..][0..2], scale, .little);
                        for (0..q2_source_block) |i| {
                            const byte = source[source_base + 2 + i / 4];
                            const code: u8 = (byte >> @intCast((i % 4) * 2)) & 3;
                            if (code == 3) return error.ReservedTernaryCode;
                        }
                    }
                    for (0..q8_subblocks) |sub| {
                        ...
                        for (0..2) |code_beat| {
                            const dst = block_base + (1 + sub * 2 + code_beat) * weight_port_beat_bytes + lane * 4;
                            const src = source_base + 2 + source_sub * 8 + code_beat * 4;
                            @memcpy(out[dst..][0..4], source[src..][0..4]);
                        }
                    }
                }
            }
...
/// Pack Q, K, and V Q2_0 matrices into one GQA-grouped projection stream.
/// For each KV head the logical rows are its mapped Q heads, then its K head,
/// then its V head. This is the order consumed by the resident attention path.
pub fn packFusedQkvWeightsFromGgmlQ2_0(
    q_heads: usize,
    kv_heads: usize,
    head_dim: usize,
    input_k: usize,
    ...
    const shape = try fusedQkvShape(q_heads, kv_heads, head_dim);
    const q1_blocks = try blocksPerRow(input_k);
    const source_blocks_per_row = input_k / q2_source_block;
    const source_row_bytes = source_blocks_per_row * q2_source_block_bytes;
    if (q_weights.len != shape.q_rows * source_row_bytes or
        k_weights.len != shape.kv_rows * source_row_bytes or
        v_weights.len != shape.kv_rows * source_row_bytes)
        return error.InvalidLength;
    if (out.len != try packedTernaryWeightBytes(shape.rows, input_k))
        return error.InvalidLength;

    @memset(out, 0);
    for (0..weight_ports) |port| {
        for (0..rowblocksFor(shape.rows)) |rb| {
            const row_start = rb * rows_per_block + port * rows_per_port;
            const row_count = if (row_start < shape.rows) @min(rows_per_port, shape.rows - row_start) else 0;
            for (0..q1_blocks) |q1| {
                const block_base = ternaryWeightBlockOffset(shape.rows, input_k, port, rb, q1);
                for (0..row_count) |lane| {
                    const source_ref = fusedQkvSourceRow(shape, head_dim, row_start + lane);
                    const source = switch (source_ref.tensor) {
                        .q => q_weights,
                        .k => k_weights,
                        .v => v_weights,
                    };
                    const src0 = source_ref.row * source_row_bytes + (q1 * 2) * q2_source_block_bytes;
                    const src1 = src0 + q2_source_block_bytes;
                    for (0..2) |half| {
                        const source_base = if (half == 0) src0 else src1;
                        const scale = std.mem.readInt(u16, source[source_base..][0..2], .little);
...
/// Pack separate GATE and UP Q1_0 matrices directly into fused resident order.
pub fn packFusedGateUpWeightsFromGgmlQ1_0(
    rows: usize,
    k: usize,
    gate_weights: []const u8,
    up_weights: []const u8,
    ...
    const source_bytes = rows * source_row_bytes;
    if (gate_weights.len != source_bytes or up_weights.len != source_bytes) return error.InvalidLength;
    if (out.len != try packedWeightBytes(fused_rows, k)) return error.InvalidLength;
    ...
    for (0..weight_ports) |port| {
        for (0..rowblocksFor(fused_rows)) |rb| {
            ...
            const row_count = if (row_start < fused_rows) @min(rows_per_port, fused_rows - row_start) else 0;
            for (0..q1_blocks) |q1| {
                const base = weightBlockOffset(fused_rows, k, port, rb, q1);
                for (0..row_count) |lane| {
                    const fused_row = row_start + lane;
                    const source = if (fused_row & 1 == 0) gate_weights else up_weights;
                    const source_row = fused_row / 2;
                    const source_offset = source_row * source_row_bytes + q1 * q1_block_bytes;
                    const scale = std.mem.readInt(u16, source[source_offset..][0..2], .little);
                    std.mem.writeInt(u16, out[base + lane * 4 ..][0..2], scale, .little);
                }
                for (0..q8_subblocks) |sub| {
                    const sub_base = base + (1 + sub) * weight_port_beat_bytes;
                    for (0..row_count) |lane| {
                        ...
                        const source = if (fused_row & 1 == 0) gate_weights else up_weights;
                        const source_row = fused_row / 2;
                        const source_offset = source_row * source_row_bytes +
                            q1 * q1_block_bytes + @sizeOf(f16) + sub * (q8_block / 8);
                        const bits = std.mem.readInt(u32, source[source_offset..][0..4], .little);
                        std.mem.writeInt(u32, out[sub_base + lane * 4 ..][0..4], bits, .little);
                    }
                }
            }
...
/// Q1_0 counterpart of `packFusedQkvWeightsFromGgmlQ2_0`.
pub fn packFusedQkvWeightsFromGgmlQ1_0(
    q_heads: usize,
    kv_heads: usize,
    head_dim: usize,
    input_k: usize,
    ...
    if (q_weights.len != shape.q_rows * source_row_bytes or
        ...
        v_weights.len != shape.kv_rows * source_row_bytes)
        return error.InvalidLength;
    if (out.len != try packedWeightBytes(shape.rows, input_k))
        return error.InvalidLength;
    ...
    for (0..weight_ports) |port| {
        for (0..rowblocksFor(shape.rows)) |rb| {
            ...
            const row_count = if (row_start < shape.rows) @min(rows_per_port, shape.rows - row_start) else 0;
            for (0..q1_blocks) |q1| {
                const base = weightBlockOffset(shape.rows, input_k, port, rb, q1);
                for (0..row_count) |lane| {
                    const source_ref = fusedQkvSourceRow(shape, head_dim, row_start + lane);
                    const source = switch (source_ref.tensor) {
                        .q => q_weights,
                    ...
                    const source_offset = source_ref.row * source_row_bytes + q1 * q1_block_bytes;
                    const scale = std.mem.readInt(u16, source[source_offset..][0..2], .little);
                    std.mem.writeInt(u16, out[base + lane * 4 ..][0..2], scale, .little);
                }
                for (0..q8_subblocks) |sub| {
                    ...
                    for (0..row_count) |lane| {
                        ...
                        const source_offset = source_ref.row * source_row_bytes +
                            q1 * q1_block_bytes + @sizeOf(f16) + sub * (q8_block / 8);
                        const bits = std.mem.readInt(u32, source[source_offset..][0..4], .little);
                        std.mem.writeInt(u32, out[sub_base + lane * 4 ..][0..4], bits, .little);
                    }
...
fn fillFusedQkvQ1Source(raw: []u8, rows: usize, input_k: usize, tag: usize) void {
    const q1_blocks = input_k / q1_block;
    for (0..rows) |row| {
        for (0..q1_blocks) |q1| {
            const off = (row * q1_blocks + q1) * q1_block_bytes;
            const scale: f16 = @floatFromInt(tag * 128 + row * q1_blocks + q1 + 1);
            std.mem.writeInt(u16, raw[off..][0..2], @bitCast(scale), .little);
            for (0..q8_subblocks) |sub| {
                const word: u32 = (@as(u32, @intCast(tag)) << 28) |
                    (@as(u32, @intCast(row)) << 8) |
                    (@as(u32, @intCast(q1)) << 4) |
                    @as(u32, @intCast(sub));
                std.mem.writeInt(u32, raw[off + 2 + sub * 4 ..][0..4], word, .little);
            }
        }
    }
}

fn fillFusedQkvQ2Source(raw: []u8, rows: usize, input_k: usize, tag: usize) void {
    const blocks = input_k / q2_source_block;
    for (0..rows) |row| {
        for (0..blocks) |block| {
            const off = (row * blocks + block) * q2_source_block_bytes;
            const scale: f16 = @floatFromInt(tag * 128 + row * blocks + block + 1);
            std.mem.writeInt(u16, raw[off..][0..2], @bitCast(scale), .little);
            for (0..q2_source_block) |i| {
                const code: u8 = @intCast((tag + row + block + i) % 3);
...
test "fused gate up q1 pack preserves native adjacent row pairs" {
    const rows = rows_per_block;
    const k = q1_block * 2;
    const q1_blocks = comptime blocksPerRow(k) catch unreachable;
    const raw_len = rows * q1_blocks * q1_block_bytes;
    const one_packed_len = comptime packedWeightBytes(rows, k) catch unreachable;
    const fused_packed_len = comptime packedFusedGateUpWeightBytes(rows, k) catch unreachable;
    try std.testing.expectEqual(one_packed_len * 2, fused_packed_len);

    var gate: [raw_len]u8 = [_]u8{0} ** raw_len;
    var up: [raw_len]u8 = [_]u8{0} ** raw_len;
    for (0..rows) |row| {
        for (0..q1_blocks) |q1| {
            const block = row * q1_blocks + q1;
            const off = block * q1_block_bytes;
            const gate_scale: f16 = @floatFromInt(row * q1_blocks + q1 + 1);
            const up_scale: f16 = @floatFromInt(100 + row * q1_blocks + q1);
            std.mem.writeInt(u16, gate[off..][0..2], @bitCast(gate_scale), .little);
            std.mem.writeInt(u16, up[off..][0..2], @bitCast(up_scale), .little);
            for (0..q8_subblocks) |sub| {
                const row_bits: u32 = @intCast(row);
                const q1_bits: u32 = @intCast(q1);
                const sub_bits: u32 = @intCast(sub);
                std.mem.writeInt(u32, gate[off + 2 + sub * 4 ..][0..4], 0x1000_0000 | row_bits << 8 | q1_bits << 4 | sub_bits, .little);
                std.mem.writeInt(u32, up[off + 2 + sub * 4 ..][0..4], 0xA000_0000 | row_bits << 8 | q1_bits << 4 | sub_bits, .little);
            }
        }
    }

    var fused: [fused_packed_len]u8 = undefined;
    try packFusedGateUpWeightsFromGgmlQ1_0(rows, k, &gate, &up, &fused);
    for (0..rows) |row| {
        for (0..q1_blocks) |q1| {
            const source_off = (row * q1_blocks + q1) * q1_block_bytes;
            const gate_scale: f16 = @bitCast(std.mem.readInt(u16, gate[source_off..][0..2], .little));
            const up_scale: f16 = @bitCast(std.mem.readInt(u16, up[source_off..][0..2], .little));
            try std.testing.expectEqual(gate_scale, try packedWeightScale(&fused, rows * 2, k, row * 2, q1));
            try std.testing.expectEqual(up_scale, try packedWeightScale(&fused, rows * 2, k, row * 2 + 1, q1));
            for (0..q8_subblocks) |sub| {
                const source_bits_off = source_off + 2 + sub * 4;
                try std.testing.expectEqual(
                    std.mem.readInt(u32, gate[source_bits_off..][0..4], .little),
                    try packedWeightBits(&fused, rows * 2, k, row * 2, q1, sub),
                );
                try std.testing.expectEqual(
                    std.mem.readInt(u32, up[source_bits_off..][0..4], .little),
                    try packedWeightBits(&fused, rows * 2, k, row * 2 + 1, q1, sub),
                );
            }
        }
    }

    // Each 128-bit port beat contains two complete GATE/UP pairs in adjacent
    // 32-bit lanes; concatenating the four ports preserves that lane order.
    for (0..rowblocksFor(rows * 2)) |rb| {
        for (0..weight_ports) |port| {
            const base = weightBlockOffset(rows * 2, k, port, rb, 0);
            for (0..rows_per_port / 2) |pair| {
                const lane = pair * 2;
                const source_row = (rb * rows_per_block + port * rows_per_port + lane) / 2;
                const gate_scale: f16 = @bitCast(std.mem.readInt(u16, fused[base + lane * 4 ..][0..2], .little));
                const up_scale: f16 = @bitCast(std.mem.readInt(u16, fused[base + (lane + 1) * 4 ..][0..2], .little));
                try std.testing.expectEqual(@as(f16, @floatFromInt(source_row * q1_blocks + 1)), gate_scale);
                try std.testing.expectEqual(@as(f16, @floatFromInt(100 + source_row * q1_blocks)), up_scale);
            }
        }
    }
}
...
test "fused gate up q2 pack preserves native adjacent row pairs" {
    const rows = rows_per_block;
    const k = q1_block;
    const source_blocks = k / q2_source_block;
    const raw_len = rows * source_blocks * q2_source_block_bytes;
    const one_packed_len = comptime packedTernaryWeightBytes(rows, k) catch unreachable;
    const fused_packed_len = comptime packedFusedGateUpTernaryWeightBytes(rows, k) catch unreachable;
    try std.testing.expectEqual(one_packed_len * 2, fused_packed_len);
    ...
    for (0..rows) |row| {
        for (0..source_blocks) |block| {
            const off = (row * source_blocks + block) * q2_source_block_bytes;
            const gate_scale: f16 = @floatFromInt(row * source_blocks + block + 1);
            const up_scale: f16 = @floatFromInt(100 + row * source_blocks + block);
            std.mem.writeInt(u16, gate[off..][0..2], @bitCast(gate_scale), .little);
            std.mem.writeInt(u16, up[off..][0..2], @bitCast(up_scale), .little);
            for (0..q2_source_block) |i| {
                const gate_code: u8 = @intCast((row + block + i) % 3);
                const up_code: u8 = @intCast((row + block + i + 1) % 3);
                gate[off + 2 + i / 4] |= gate_code << @intCast((i % 4) * 2);
                up[off + 2 + i / 4] |= up_code << @intCast((i % 4) * 2);
            }
        }
    }

    var fused: [fused_packed_len]u8 = undefined;
    try packFusedGateUpWeightsFromGgmlQ2_0(rows, k, &gate, &up, &fused);
    for (0..rows) |row| {
        for (0..source_blocks) |half| {
            const source_off = (row * source_blocks + half) * q2_source_block_bytes;
            const gate_scale: f16 = @bitCast(std.mem.readInt(u16, gate[source_off..][0..2], .little));
            const up_scale: f16 = @bitCast(std.mem.readInt(u16, up[source_off..][0..2], .little));
            try std.testing.expectEqual(gate_scale, try packedTernaryWeightScale(&fused, rows * 2, k, row * 2, 0, half));
            try std.testing.expectEqual(up_scale, try packedTernaryWeightScale(&fused, rows * 2, k, row * 2 + 1, 0, half));
            for (0..q2_source_block) |i| {
                const gate_code = (gate[source_off + 2 + i / 4] >> @intCast((i % 4) * 2)) & 3;
                const up_code = (up[source_off + 2 + i / 4] >> @intCast((i % 4) * 2)) & 3;
                const index = half * q2_source_block + i;
                try std.testing.expectEqual(@as(i2, @intCast(@as(i8, @intCast(gate_code)) - 1)), try packedTernaryWeight(&fused, rows * 2, k, row * 2, 0, index));
                try std.testing.expectEqual(@as(i2, @intCast(@as(i8, @intCast(up_code)) - 1)), try packedTernaryWeight(&fused, rows * 2, k, row * 2 + 1, 0, index));
            }
        }
    }
    ...
    for (0..rowblocksFor(rows * 2)) |rb| {
        for (0..weight_ports) |port| {
            const base = ternaryWeightBlockOffset(rows * 2, k, port, rb, 0);
            for (0..rows_per_port / 2) |pair| {
                const lane = pair * 2;
                const source_row = (rb * rows_per_block + port * rows_per_port + lane) / 2;
                const gate_scale: f16 = @bitCast(std.mem.readInt(u16, fused[base + lane * 4 ..][0..2], .little));
                const up_scale: f16 = @bitCast(std.mem.readInt(u16, fused[base + (lane + 1) * 4 ..][0..2], .little));
                try std.testing.expectEqual(@as(f16, @floatFromInt(source_row * source_blocks + 1)), gate_scale);
                try std.testing.expectEqual(@as(f16, @floatFromInt(100 + source_row * source_blocks)), up_scale);
            }
        }
    }
}

test "fused QKV pack preserves GQA group order for Q1 and Q2" {
    const q_heads = 4;
    const kv_heads = 2;
    const head_dim = 8;
    const input_k = q1_block;
    const shape = comptime fusedQkvShape(q_heads, kv_heads, head_dim) catch unreachable;
    const q1_q_len = shape.q_rows * q1_block_bytes;
    const q1_kv_len = shape.kv_rows * q1_block_bytes;
    const q2_q_len = shape.q_rows * 2 * q2_source_block_bytes;
    const q2_kv_len = shape.kv_rows * 2 * q2_source_block_bytes;
    const q1_out_len = comptime packedFusedQkvWeightBytes(q_heads, kv_heads, head_dim, input_k) catch unreachable;
    const q2_out_len = comptime packedFusedQkvTernaryWeightBytes(q_heads, kv_heads, head_dim, input_k) catch unreachable;

    try std.testing.expectEqual(
        (try packedWeightBytes(shape.q_rows, input_k)) +
            2 * (try packedWeightBytes(shape.kv_rows, input_k)),
        q1_out_len,
    );
    try std.testing.expectEqual(
        (try packedTernaryWeightBytes(shape.q_rows, input_k)) +
            2 * (try packedTernaryWeightBytes(shape.kv_rows, input_k)),
        q2_out_len,
    );

    var q1_q: [q1_q_len]u8 = [_]u8{0} ** q1_q_len;
    var q1_k: [q1_kv_len]u8 = [_]u8{0} ** q1_kv_len;
    var q1_v: [q1_kv_len]u8 = [_]u8{0} ** q1_kv_len;
    fillFusedQkvQ1Source(&q1_q, shape.q_rows, input_k, 0);
    fillFusedQkvQ1Source(&q1_k, shape.kv_rows, input_k, 1);
    fillFusedQkvQ1Source(&q1_v, shape.kv_rows, input_k, 2);
    var q1_out: [q1_out_len]u8 = undefined;
    try packFusedQkvWeightsFromGgmlQ1_0(
        q_heads,
        kv_heads,
        head_dim,
        input_k,
    ...
    var q2_q: [q2_q_len]u8 = [_]u8{0} ** q2_q_len;
    var q2_k: [q2_kv_len]u8 = [_]u8{0} ** q2_kv_len;
    var q2_v: [q2_kv_len]u8 = [_]u8{0} ** q2_kv_len;
    fillFusedQkvQ2Source(&q2_q, shape.q_rows, input_k, 0);
    fillFusedQkvQ2Source(&q2_k, shape.kv_rows, input_k, 1);
    fillFusedQkvQ2Source(&q2_v, shape.kv_rows, input_k, 2);
    var q2_out: [q2_out_len]u8 = undefined;
    try packFusedQkvWeightsFromGgmlQ2_0(
        q_heads,
        kv_heads,
        head_dim,
        input_k,
    ...
    for (0..shape.rows) |fused_row| {
        const source_ref = fusedQkvSourceRow(shape, head_dim, fused_row);
        const tag: usize = switch (source_ref.tensor) {
            .q => 0,
            .k => 1,
            .v => 2,
        };
        const expected_q1_scale: f16 = @floatFromInt(tag * 128 + source_ref.row + 1);
        try std.testing.expectEqual(
            expected_q1_scale,
            try packedWeightScale(&q1_out, shape.rows, input_k, fused_row, 0),
        );
        for (0..q8_subblocks) |sub| {
            const expected: u32 = (@as(u32, @intCast(tag)) << 28) |
                (@as(u32, @intCast(source_ref.row)) << 8) |
                @as(u32, @intCast(sub));
            try std.testing.expectEqual(
                expected,
                try packedWeightBits(&q1_out, shape.rows, input_k, fused_row, 0, sub),
            );
        }

        for (0..2) |half| {
            const expected_q2_scale: f16 = @floatFromInt(
                tag * 128 + source_ref.row * 2 + half + 1,
            );
            try std.testing.expectEqual(
                expected_q2_scale,
                try packedTernaryWeightScale(&q2_out, shape.rows, input_k, fused_row, 0, half),
            );
        }
        for (0..q1_block) |index| {
            const block = index / q2_source_block;
            const local = index % q2_source_block;
            const code: u8 = @intCast((tag + source_ref.row + block + local) % 3);
            const expected: i2 = @intCast(@as(i8, @intCast(code)) - 1);
            try std.testing.expectEqual(
                expected,
                try packedTernaryWeight(&q2_out, shape.rows, input_k, fused_row, 0, index),
            );
        }
    }
}
